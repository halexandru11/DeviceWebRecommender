...
export async function insertWishlistProduct(product, username) {
  try {
    ...
    const simpleUrl = url.substring(url.indexOf('://') + 3).replace('www.', '');
    ...
    try {
      ...
      const productValues = productList.map((product) => {
        ...
        const simpleUrl = url
          ...
          .replace('www.', '');
        const vendorName = simpleUrl.substring(0, simpleUrl.indexOf('.')) || '';
        return [null, url, name, price, rating, numReviews, vendorName];
      });

      const [insertProductResults] = await connection.query(
        insertProductQuery,
        [productValues]
      );
      ...
      for (let i = 0; i < productList.length; i++) {
        ...
        const imageUrl =
          productList[i].img || 'https://www.emag.ro/default-image.jpg';
      ...
      const values = await Promise.all(
        productList.map(async (product) => {
          ...
          const simpleUrl = url
            ...
            .replace('www.', '');
          const vendorName =
            simpleUrl.substring(0, simpleUrl.indexOf('.')) || '';
          // const url = new URL(product.url);
          //const vendorName = url.hostname.split('.')[0];
